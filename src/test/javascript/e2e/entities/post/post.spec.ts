...
describe('Post e2e test', () => {
  ...
  it('should load Posts', async () => {
    ...
    expect(await postComponentsPage.getTitle()).to.eq('periodApp.post.home.title');
  });

  it('should load create Post page', async () => {
    await postComponentsPage.clickOnCreateButton();
    postUpdatePage = new PostUpdatePage();
    expect(await postUpdatePage.getPageTitle()).to.eq('periodApp.post.home.createOrEditLabel');
  ...
  /*  it('should create and save Posts', async () => {
        ...
        expect(await postUpdatePage.getCreationDateInput()).to.eq('2000-12-31', 'Expected creationDate value to be equals to 2000-12-31');
  ...
  /*  it('should delete last Post', async () => {
        ...
        expect(await postDeleteDialog.getDialogTitle())
            .to.eq('periodApp.post.delete.question');
